...
describe("SettleOverdueProject", () => {
    it("Test settle overdue project with correct project ID should success", async () => {
        ...
        const prevBalance = await ethers.provider.getBalance(donor.account.address)

        await NoneMoney.write.settleOverdueProject([0n], {
            account: donor.account
        })

        const donatedBalance = await ethers.provider.getBalance(donor.account.address)
    ...
    it("Test settle overdue project with negative project ID should be revert", async () => {
        ...
        await expect(NoneMoney.write.settleOverdueProject([-1n], {
            account: donor.account
        })).to.be.throw
    })
    it("Test settle overdue project with absent project ID should be revert", async () => {
        ...
        await expect(NoneMoney.write.settleOverdueProject([3n], {
            account: donor.account
        })).to.be.throw
    })
    it("Test settle overdue project with absent project ID should be revert", async () => {
    ...
    it("Test settle overdue project with repeat settle should revert", async () => {
        ...
        await expect(NoneMoney.write.settleOverdueProject([0n], {
            account: donor.account
        })).to.be.throw
    })
})